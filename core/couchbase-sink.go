...
func NewCouchbaseSink(baseURL, bucketName string, numConnections int) (MCDSink, error) {
	...
	h := sink {
		...
		connsPerDownstream: numConnections,
	}
	for i, hostname := range serverList {
		allChans := make([]chan request, numConnections)
